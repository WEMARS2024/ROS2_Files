...
class Control_Receiver_Node(Node):
    ...
    def __init__(self):
        ...
        self.serial = serial.Serial(device="/dev/ttyUSB0", baud=921600)
    ...
    def send_motor_data(self, current_value):

        motor_left = f"b'(ML,{current_value[0]:.2f})\n'".encode()
        motor_right = f"b'(MR,{current_value[0]:.2f}\n'".encode()

        self.serial.write(motor_left)
        self.serial.write(motor_right)
